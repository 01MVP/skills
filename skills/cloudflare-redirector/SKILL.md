...
1. 检查前提：已安装 Node.js 与 pnpm，且已配置 `CLOUDFLARE_API_TOKEN`。
2. 更新规则文件：编辑 `data/redirects.json`。
3. 更新路由：编辑 `wrangler.jsonc` 的 `routes` 与 `account_id`。
4. 先预览再执行：
   - `pnpm run dns:sync:dry-run`
   - `pnpm run deploy:dry-run`
5. 正式发布：`pnpm run dns:sync && pnpm run deploy`。

## Files To Edit

- `data/redirects.json`: 重定向规则（`source`, `destination`, `status`）
- `wrangler.jsonc`: Worker 入口、账号、路由
